...
const socket = io('https://wbserver.onrender.com');
...
export default function Canvas({ roomID, load, user }) {
  ...
  const saveSession = async () => {
    ...
    try {
      const response = await fetch('https://wbserver.onrender.com/save-session', {
        method: 'POST',
  ...
  const loadSession = async () => {
    try {
      const response = await fetch(`https://wbserver.onrender.com/load-session/${roomID}`);
      if (response.ok) {
